...
using KitchenPC.Core.Categorization;
...
internal class MockCategorizationDBLoader : IDBLoader
{
   public IEnumerable<IIngredientCommonality> LoadCommonIngredients()
   {
      ...
      {
         new(Mock.Ingredients.SALT.Id, 1f),
         new(Mock.Ingredients.GRANULATED_SUGAR.Id, 0.90f),
         new(Mock.Ingredients.EGGS.Id, 0.88f),
         new(Mock.Ingredients.ALL_PURPOSE_FLOUR.Id, 0.76f),
         new(Mock.Ingredients.SALTED_BUTTER.Id, 0.63f),
         new(Mock.Ingredients.VANILLA_EXTRACT.Id, 0.50f),
         new(Mock.Ingredients.WATER.Id, 0.40f),
         new(Mock.Ingredients.BLACK_PEPPER.Id, 0.35f),
...
public class Categorization
{
   ...
   {
      engine.Categorize(Mock.Recipes.BEST_BROWNIES);
   }
