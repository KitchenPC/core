...
{
   public Single? SizeLow { get; set; }
   public Single SizeHigh { get; set; }
   ...
   public static Amount Normalize(Amount amount, float multiplier)
   {
      ...
      if (Core.Unit.GetConvType(ret.Unit) == UnitType.Weight)
      {
      ...
      if (Core.Unit.GetConvType(ret.Unit) == UnitType.Volume)
      {
   ...
   public static Amount operator +(Amount a1, Amount a2)
   {
      ...
      if (UnitConverter.CanConvert(a1.Unit, a2.Unit))
      {
         ...
         var newLow = a2.SizeLow.HasValue
            ? (float?)UnitConverter.Convert(a2.SizeLow.Value, a2.Unit, a1.Unit)
            : null;
   ...
   public override string ToString()
   {
      return ToString(
         (SizeLow.HasValue || SizeHigh > 1)
            ? Core.Unit.GetPlural(Unit)
            : Core.Unit.GetSingular(Unit)
      );
   }
   ...
   public string ToString(string unit)
   {
      ...
      if (Core.Unit.GetConvType(Unit) == UnitType.Weight) //Render in decimal
      {
