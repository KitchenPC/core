...
public class StaticContext : IKPCContext, IProvisionTarget, IProvisionSource
{
   ...
   private ISearchProvider searchProvder;
   ...
   {
      get { return new StaticContext().builder; }
   }
   ...
   public void Initialize()
   {
      ...
      if (!File.Exists(path))
         throw new FileNotFoundException(
            "Could not initialize StaticContext.  Data file not found.",
            path
         );
      ...
      using (var fileReader = new FileStream(path, FileMode.Open))
      {
         ...
         if (store == null)
            throw new DataStoreException(
               "Could not deserialize data store.  It might be correct or an invalid format."
            );
      }

      // Initialize ingredient parser
      ingParser = new IngredientParser();
      ingParser.CreateIndex(
         store.Ingredients.Select(i => new IngredientSource(i.IngredientId, i.DisplayName))
      );
   ...
   public IKPCContext AsUserContext(AuthIdentity userContext)
   {
      throw new NotImplementedException(
         "StaticContext does not support cloning under a specific user identity."
      );
   }
   ...
   {
      get { return new StaticModelerLoader(store); }
   }
   ...
   public Recipe[] ReadRecipes(Guid[] recipeIds, ReadRecipeOptions options)
   {
      ...
      foreach (var r in recipes)
      {
         ...
         {
            var userRating = store.RecipeRatings.SingleOrDefault(ur =>
               (ur.RecipeId == r.RecipeId && ur.UserId == Identity.UserId)
            );
            recipe.UserRating = userRating != null ? (Rating)userRating.Rating : Rating.None;
         }

         recipe.Ingredients = riIndex[r.RecipeId]
            .Select(i => new IngredientUsage
            {
               Amount = i.Qty.HasValue ? new Amount(i.Qty.Value, i.Unit) : null,
               PrepNote = i.PrepNote,
               Section = i.Section,
               Form = i.IngredientFormId.HasValue
                  ? IngredientForms.ToIngredientForm(formIndex[i.IngredientFormId.Value])
                  : null,
               Ingredient = Provisioning.DTO.Ingredients.ToIngredient(
                  ingIndex[i.IngredientId],
                  imetaIndex[i.IngredientId]
               ),
            })
            .ToArray();
   ...
   {
      var userRating = store.RecipeRatings.SingleOrDefault(ur =>
         (ur.RecipeId == recipeId && ur.UserId == Identity.UserId)
      );
      if (userRating == null)
      {
         store.RecipeRatings.Add(
            new RecipeRatings
            {
   ...
   public RecipeResult CreateRecipe(Recipe recipe)
   {
      ...
      return new RecipeResult { RecipeCreated = true, NewRecipeId = recipe.Id };
   }
   ...
   {
      var existing = store
         .QueuedRecipes.Where(r => r.UserId == Identity.UserId)
   ...
   public void EnqueueRecipes(params Guid[] recipeIds)
   {
      ...
      store.QueuedRecipes.AddRange(
         noDupes.Select(r => new QueuedRecipes
         {
   ...
   public RecipeBrief[] GetRecipeQueue()
   {
      ...
      return queue
         ...
         .Select(Provisioning.DTO.Recipes.ToRecipeBrief)
         .ToArray();
   }
   ...
   {
      var forms = store
         .IngredientForms.Where(f => f.IngredientId == id)
   ...
   {
      var ing = store.Ingredients.FirstOrDefault(i =>
         String.Compare(i.DisplayName, ingredient, StringComparison.OrdinalIgnoreCase) == 0
      );
      return ing == null ? null : Provisioning.DTO.Ingredients.ToIngredient(ing);
   }
   ...
   {
      var ingIndex = store.GetIndexedIngredients();
      Provisioning.DTO.Ingredients dtoIng;

      return ingIndex.TryGetValue(ingid, out dtoIng)
         ? Provisioning.DTO.Ingredients.ToIngredient(dtoIng)
         : null;
   }
   ...
   {
      var loadDef = true;
      var query = store.ShoppingLists.Where(p => p.UserId == Identity.UserId);
      ...
      if (!options.LoadItems) // We're done!
      {
         return (loadDef ? new ShoppingList[] { ShoppingList.Default } : new ShoppingList[0])
            .Concat(dbLists.Select(Provisioning.DTO.ShoppingLists.ToShoppingList))
            .ToArray();
      }

      // All user's shopping list items
      var dbItems = store.ShoppingListItems.Where(p => p.UserId == Identity.UserId).ToList();
      ...
      ret.AddRange(dbLists.Select(Provisioning.DTO.ShoppingLists.ToShoppingList));

      // Add items to list
      foreach (var list in ret)
      {
         var itemsInList = dbItems.Where(p => p.ShoppingListId.Equals(list.Id));
         var items = itemsInList.Select(item => new ShoppingListItem(item.ItemId)
         {
            Raw = item.Raw,
            Ingredient = item.IngredientId.HasValue
               ? Provisioning.DTO.Ingredients.ToIngredient(
                  indexIngredients[item.IngredientId.Value]
               )
               : null,
            Recipe = item.RecipeId.HasValue
               ? Provisioning.DTO.Recipes.ToRecipeBrief(indexRecipes[item.RecipeId.Value])
               : null,
   ...
   public ShoppingListResult CreateShoppingList(ShoppingList list)
   {
      ...
      var dbList = new Provisioning.DTO.ShoppingLists
      {
   ...
   )
   {
      ...
      {
         var dbDeletes = store
            .ShoppingListItems.Where(p => p.UserId == Identity.UserId)
      ...
      Provisioning.DTO.ShoppingLists dbList = null;
      List<ShoppingListItems> dbItems = null;
      if (listId.HasValue)
      {
         dbList = store
            .ShoppingLists.Where(p => p.UserId == Identity.UserId)
      ...
      toAdd.ForEach(item =>
      {
         ...
         if (source.Ingredient != null && source.Amount == null) // Raw ingredient without any amount
         {
            var existingItem = dbItems.FirstOrDefault(i =>
               i.IngredientId.HasValue && i.IngredientId.Value == source.Ingredient.Id
            );
      ...
      var items = store
         .ShoppingListItems.Where(p => p.UserId == Identity.UserId)
         .Where(l => l.ShoppingListId == shoppingListId)
         .Select(item => new ShoppingListItem(item.ItemId)
         {
            Raw = item.Raw,
            Ingredient = item.IngredientId.HasValue
               ? Provisioning.DTO.Ingredients.ToIngredient(
   ...
   public Menu[] GetMenus(IList<Menu> menus, GetMenuOptions options)
   {
      ...
      ret.AddRange(dbMenus.Select(Provisioning.DTO.Menus.ToMenu));
      ...
      return ret.Select(m => new Menu(m)
         {
            Recipes = (
               ...
               .Select(r => Provisioning.DTO.Recipes.ToRecipeBrief(indexRecipes[r.RecipeId]))
               .ToArray(),
         })
         .ToArray();
   }
   ...
   public void DeleteMenus(params Guid[] menuIds)
   {
      var dbFavorites = store
         .Favorites.Where(p => p.UserId == Identity.UserId)
         .Where(p => p.MenuId.HasValue)
         .Where(p => menuIds.Contains(p.MenuId.Value));

      var dbMenus = store
         .Menus.Where(p => p.UserId == Identity.UserId)
   ...
   )
   {
      ...
      Provisioning.DTO.Menus dbMenu = null;
      if (menuId.HasValue)
      {
      ...
      if (recipesMove.Any()) // Move items to another menu
      {
         ...
         {
            Provisioning.DTO.Menus dbTarget = null;
   ...
   public MenuResult CreateMenu(Menu menu, params Guid[] recipeIds)
   {
      ...
      store.Menus.Add(
         dbMenu = new Provisioning.DTO.Menus
         {
            MenuId = Guid.NewGuid(),
            UserId = Identity.UserId,
            Title = menu.Title,
   ...
   public void InitializeStore()
   {
      if (String.IsNullOrWhiteSpace(DataDirectory))
         throw new InvalidConfigurationException(
            "StaticContext requires a configured data directory."
         );
