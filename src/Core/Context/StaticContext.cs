...
public class StaticContext : IKPCContext, IProvisionTarget, IProvisionSource
{
    ...
    {
        throw new NotImplementedException("StaticContext does not support cloning under a specific user identity.");
    }
    ...
    public Recipe[] ReadRecipes(Guid[] recipeIds, ReadRecipeOptions options)
    {
        ...
        foreach (var r in recipes)
        {
            ...
            recipe.Ingredients = riIndex[r.RecipeId].Select(i => new IngredientUsage
            {
                ...
                Ingredient = Provisioning.DTO.Ingredients.ToIngredient(ingIndex[i.IngredientId], imetaIndex[i.IngredientId])
            }).ToArray();
    ...
    public RecipeResult CreateRecipe(Recipe recipe)
    {
        ...
        // Recipes
        store.Recipes.Add(new Provisioning.DTO.Recipes
        {
            RecipeId = recipe.Id,
            Title = recipe.Title,
    ...
    public Ingredient ReadIngredient(Guid ingid)
    {
        ...
        return ingIndex.TryGetValue(ingid, out dtoIng) ? Provisioning.DTO.Ingredients.ToIngredient(dtoIng) : null;
    }
    ...
    public ShoppingList[] GetShoppingLists(IList<ShoppingList> lists, GetShoppingListOptions options)
    {
        ...
        foreach (var list in ret)
        {
            ...
            var items = itemsInList.Select(item =>
                ...
                {
                    Raw = item.Raw,
                    Ingredient = item.IngredientId.HasValue ? Provisioning.DTO.Ingredients.ToIngredient(indexIngredients[item.IngredientId.Value]) : null,
                    Recipe = item.RecipeId.HasValue ? Provisioning.DTO.Recipes.ToRecipeBrief(indexRecipes[item.RecipeId.Value]) : null,
    ...
    private ShoppingListResult UpdateShoppingList(Guid? listId, Guid[] toRemove, IEnumerable<ShoppingListModification> toModify, IEnumerable<IShoppingListSource> toAdd, string newName = null)
    {
        // Deletes
        if (toRemove.Any())
        {
        ...
        return ret.Select(m =>
            new Menu(m)
            {
                Recipes = (m.Id.HasValue
                    ...
                    ).Select(r => Provisioning.DTO.Recipes.ToRecipeBrief(indexRecipes[r.RecipeId])).ToArray()
            }).ToArray();
    }
    ...
    public MenuResult CreateMenu(Menu menu, params Guid[] recipeIds)
    {
        ...
        store.Menus.Add(dbMenu = new Provisioning.DTO.Menus
        {
            MenuId = Guid.NewGuid(),
            UserId = Identity.UserId,
            Title = menu.Title,
