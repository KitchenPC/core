...
public class CategorizationEngine
{
   ...
   public CategorizationEngine(IDBLoader loader)
   {
      ...
      commonIngs = (from i in loader.LoadCommonIngredients() select i).ToDictionary(k =>
         k.IngredientId
      );
   ...
   private void CategorizeSkill(Recipe recipe, CategorizationResult result)
   {
      ...
      result.Commonality = Convert.ToSingle(
         result.Skill_Common
            ? recipe.Ingredients.Average(i => commonIngs[i.Ingredient.Id].Commonality)
            : 0f
      );
