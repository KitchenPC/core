...
public abstract class Result
{
   ...
   public static Result BuildResult(Template template, string input, MatchData matchdata)
   {
      ...
      var ingName =
         (matchdata.Ingredient.Parent == null)
            ? matchdata.Ingredient.IngredientName
            : matchdata.Ingredient.Parent.IngredientName;
      ...
      result.PrepNote = matchdata.Preps.HasValue
         ? matchdata.Preps.ToString()
         : template.DefaultPrep;
      var pairings = matchdata.Ingredient.Pairings;

      NlpTracer.Trace(
         TraceLevel.Debug,
         "[BuildResult] Ingredient: {0}",
         matchdata.Ingredient.IngredientName
      );
      if (matchdata.Ingredient.Parent != null)
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] Re-Link to Root Ingredient: {0}",
            matchdata.Ingredient.Parent.IngredientName
         );
      }
      ...
      if (matchdata.Unit is CustomUnitNode)
      {
         IngredientForm form;
         if (
            UnitSynonyms.TryGetFormForIngredient(
               matchdata.Unit.Name,
               matchdata.Ingredient.Id,
               out form
            )
         )
         {
            NlpTracer.Trace(
               TraceLevel.Debug,
               "[BuildResult] Based on unit name {0}, linking to form id {1}",
               matchdata.Unit.Name,
               form.FormId
            );
            result.Form = form;
         ...
         else
         {
            NlpTracer.Trace(
               TraceLevel.Debug,
               "[BuildResult] ERROR: Unable to find link between unit '{0}' and ingredient '{1}'.",
               matchdata.Unit.Name,
      ...
      else
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] No custom unit found, so cannot get form based on unit."
         );
      }
      ...
      if (matchdata.Form != null)
      {
         IngredientForm form;
         if (
            FormSynonyms.TryGetFormForIngredient(
               matchdata.Form.FormName,
               matchdata.Ingredient.Id,
               out form
            )
         )
         {
            NlpTracer.Trace(
               TraceLevel.Debug,
               "[BuildResult] Based on reference to form {0}, linking to form id {1}",
               matchdata.Form.FormName,
               form.FormId
            );
            result.Form = form;
         ...
         else
         {
            NlpTracer.Trace(
               TraceLevel.Debug,
               "[BuildResult] ERROR: Unable to find link between form '{0}' and ingredient '{1}.",
               matchdata.Form.FormName,
               result.Ingredient.Name
            );
      ...
      else
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] No known form found, so cannot get form based on form synonym."
         );
      }
      ...
      if (result.Form == null) //Load default form for parsed unit type
      {
         if (matchdata.Unit == null || matchdata.Unit.Unit == Units.Unit) //TODO: Is second part necessary? Only Units.Unit would be custom form types, and we'd have errored out already if that didn't match
         {
            ...
            NlpTracer.ConditionalTrace(
               pairings.HasUnit,
               TraceLevel.Debug,
               "[BuildResult] Linking to default Unit paired form {0}",
               pairings.Unit
            );
         }
         else
            ...
            switch (Unit.GetConvType(matchdata.Unit.Unit))
            {
               case UnitType.Volume:
                  ...
                  NlpTracer.ConditionalTrace(
                     pairings.HasVolume,
                     TraceLevel.Debug,
                     "[BuildResult] Linking to default paired Volume form {0}",
                     pairings.Volume
                  );
                  break;
               case UnitType.Weight:
                  ...
                  NlpTracer.ConditionalTrace(
                     pairings.HasWeight,
                     TraceLevel.Debug,
                     "[BuildResult] Linking to default paired Weight form {0}",
                     pairings.Weight
                  );
                  break;
            }
         ...
         if (result.Form == null)
         {
            NlpTracer.Trace(
               TraceLevel.Debug,
               "[BuildResult] Could not find any default pairing for the unit type: {0}",
               result.Amount.Unit
            );
         }
      }
      ...
      if (result.Form != null && parsedType == Unit.GetConvType(result.Form.FormUnitType))
      {
         NlpTracer.Trace(
            TraceLevel.Info,
            "[BuildResult] SUCCESS: Linked form is compatible with usage reference."
         );
      ...
      if (parsedType == UnitType.Volume && matchdata.Preps.HasValue)
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] Checking for form matching prep note: {0}",
            matchdata.Preps
         );
      ...
      else
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] Could not clarify form through prep note, since unit type is not volumetric or there is no prep note."
         );
      }
      ...
      if (result.Form != null)
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] Form and unit incompatible - attempting to auto-convert form {0}",
            result.Form
         );
         ...
         else if (parsedType == UnitType.Unit && formType == UnitType.Volume) //Something like 3 mashed bananas
         {
            if (
               pairings.HasUnit
               && (matchdata.Unit == null || String.IsNullOrEmpty(matchdata.Unit.Name))
      ...
      else
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] Could not auto-convert form since there is no form to convert."
         );
      }
      ...
      if (result.Form == null) //Still have not found a form, we give up now
      {
         NlpTracer.Trace(
            TraceLevel.Debug,
            "[BuildResult] ERROR: Anomalous parsing could still not find a form for this usage."
         );
      ...
      NlpTracer.Trace(
         TraceLevel.Debug,
         "[BuildResult] ERROR: Anomalous parsing could not fix form/unit incompatibility."
      );
